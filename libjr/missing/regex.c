...
/*
** 1/26/07: JR: To install, all copy regex.h into the vendor include directory
*/

char codeset[]	= "";		/* 1/26/07: JR added for compiler error */
...
/*
** 1/26/07: More JR additions
*/
#define malloc(s)			jr_malloc(s)
#define realloc(p,s)		jr_realloc(p,s)
#define calloc(n,s)			jr_calloc(n,s)
#define free(p)				jr_free(p)
#define strdup(s)			jr_strdup(s)
#define strndup(s)			jr_strndup(s)
...
size_t
regerror (errcode_value, preg, errbuf, errbuf_size)
    int errcode_value;
    const regex_t *preg;
    char *errbuf;
    size_t errbuf_size;
{
  ...
  if (BE (errcode_value < 0
	  || errcode_value >= (int) (sizeof (__re_error_msgid_idx)
  ...
  msg = gettext (__re_error_msgid + __re_error_msgid_idx[errcode_value]);
...
free_dfa_content (re_dfa_t *dfa)
{
  ...
  if (dfa->state_table)
    for (i = 0; i <= (int) dfa->state_hash_mask; ++i)
      {
...
static unsigned re_copy_regs (struct re_registers *regs, regmatch_t *pmatch,
			      unsigned nregs, int regs_allocated) internal_function;
     ...
     int fl_backtrack;
{
  ...
  prev_idx_match = (regmatch_t *) malloc (sizeof (regmatch_t) * real_nmatch);
     ...
     const re_node_set *candidates;
{
  ...
  memset (&local_sctx.limits, 0, sizeof (local_sctx.limits));
  /*
  ** 1/26/07: JR shutup compiler warning
  */
     ...
     re_match_context_t *mctx;
{
  ...
  while (err == (reg_errcode_t *) REG_NOERROR && cur_state == NULL);
  return cur_state;
}
